// Original code taken from ---> OpenPose C++ API Tutorial - Example 7 - Face from Image
// Reads an image and the hand location, process it, and displays the hand keypoints.
// Includes all the OpenPose configuration flags.
...
void configureWrapper(op::Wrapper& opWrapper) {
  try {
    ...
    // logging_level
    op::check(
      0 <= FLAGS_logging_level && FLAGS_logging_level <= 255,
    ...
    // heatmaps to add
    const auto heatMapTypes = op::flagsToHeatMaps(
...
int tutorialApiCpp() {
  try {
    ...
    // Info
    op::log(
      "NOTE: In addition with the user flags, this demo has auto-selected the following flags:\n"
      "\t`--body_disable --hand --hand_detector 2`",
      op::Priority::High
    );
